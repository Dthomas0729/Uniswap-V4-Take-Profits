...
contract TakeProfitsHookTest is Test, Deployers {
    ...
    function _stubValidateHookAddress() private {
        ...
        // Define the controllerGasLimit value
        uint256 controllerGasLimit = 1000000; // Example value
        PoolManager poolManager = new PoolManager(controllerGasLimit);
        ...
        // Ensure that the hook address is correctly validated
        require(address(hook) != address(0), "HookAddressNotValid");
    }

    function logSender() public {
                address mememe = msg.sender;
                emit LogSender(mememe);
            }
    ...
    function setUp() public {
        ...
        bytes32 salt = keccak256(abi.encodePacked("seasalt"));
