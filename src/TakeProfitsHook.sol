...
contract TakeProfitsHook is BaseHook, ERC1155, IERC1155Receiver {
    ...
    function _tryFulfillingOrders(
        ...
        int24 currentTick = poolManager.initialize(key, params.sqrtPriceLimitX96, Constants.ZERO_BYTES);
        ...
        if (lastTickLower < currentTickLower) {
            ...
            for (int24 tick = lastTickLower; tick < currentTickLower; ) {
                ...
                if (swapAmountIn > 0) {
                    ...
                    currentTick = poolManager.initialize(key, params.sqrtPriceLimitX96, Constants.ZERO_BYTES);
    ...
    function afterSwap(
        ...
        return (TakeProfitsHook.afterSwap.selector, balanceDelta);
    }
